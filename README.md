...
已提供打包好的 Windows 绿色版，**无需安装任何环境**：

👉 **[点此进入下载页 → ArrowGod.exe（约 26 MB）](https://github.com/cheng0673/arrow-game-cz/releases/tag/v1.0.0)**

使用步骤：

1. 点击上面链接，在页面底部「Assets」中下载 **ArrowGod.exe**
2. 双击即可运行
3. 若 Windows 弹出 SmartScreen 蓝色提示，点击 **「更多信息」→「仍要运行」** 即可（自打包程序未做数字签名，属正常现象）
4. 游戏存档会保存在 exe 所在的同一目录下，删除 exe 即完全卸载

> ⏳ 单文件版首次启动需要几秒自解压，请耐心等待。以下为完整源码与源码运行方式。
